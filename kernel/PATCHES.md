...
- `patches/0521-mmc-sdhci-msm-mask-controller-irqs-while-runtime-suspended.patch`
  source: armada
  upstream: local
  notes: Fixes armada#274 symptom 2 (mmc0 bursts of 15-50 irq/s on CPU0 while runtime_status=suspended; measured on Odin 2 with ftrace: one sdhci_irq() call per counted IRQ, zero mmc errors, storm only while the host is runtime suspended — during I/O the rate is exactly 2 irq per request). Root cause: the SDCC apps clock is a shared RCG which clk_rcg2_shared_disable() parks on the always-on bi_tcxo safe source instead of gating, so the controller stays clocked while suspended and keeps asserting status events; sdhci_irq() returns IRQ_NONE early once runtime_suspended is set so nothing acks or masks the source. The patch masks SDHCI_SIGNAL_ENABLE/SDHCI_INT_ENABLE (keeping SDHCI_INT_CARD_INT for SDIO wake) in runtime_suspend and restores them from the untouched in-memory host->ier in runtime_resume. Odin 2 hotplug keeps working through its gpio card-detect. Replaces an earlier broken `prepare`-callback attempt (never applied cleanly against 7.2-rc7, reverted). Pocknix 0210's delayed clk-gating work + 0212 floor-ops revert were investigated and rejected: 0212's ops swap only changes rate rounding (same .disable park in both), it is a card-init companion to their downstream driver, not a PM fix.
...
- `patches/0540-arm64-dts-qcom-sm8550-experimental-system-pd-and-deepest-idle.patch`
  source: armada
  upstream: local (port of the SM8750 system_pd pattern, torvalds master sm8750.dtsi:208-214,319-331,5743)
  notes: EXPERIMENTAL — branch sm8550-systempd-exp only, do not merge to main without on-device validation (armada#274). SM8550 dtsi has no system-level power domain (apps_rsc votes on cluster_pd), so s2idle can structurally never reach AOSD/CXSD — qcom_stats aosd=cxsd=0 on Odin 2/Thor/RP6. Adds domain_ss3 (arm,psci-suspend-param 0x0200c354, unverified on SM8550 silicon; fallbacks 0x0200c344/0x4200c344), system_pd above cluster_pd, rewires cluster_pd and apps_rsc to system_pd. Must stay AFTER 0513+0520 (PCIe suspend-OPP floor): wake from DDR/LLCC-off hard-resets without it. Validation ladder on Odin 2: (1) clean boot + cpuidle domains up, (2) qcom_stats aosd/cxsd increment during s2idle, (3) clean resume, (4) 60-100 mA soak vs 128-170 mA stock. Pure DT addition, clean revert if the AOP rejects the param.
